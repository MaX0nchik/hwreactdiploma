...
const SearchProducts = () => {
    ...
    const submitHandler = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        navigate("/catalog.html" + setApiParamsString({categoryID,offset,query:search}));
        dispatch(setApiParams({query: search}));
    };

    return(
        <form className="catalog-search-form form-inline" onSubmit={submitHandler}>
            <input className="form-control" placeholder="Поиск" value={search} onChange={(event)=>{setSearch(event.target.value);}} />
        </form>
    )
