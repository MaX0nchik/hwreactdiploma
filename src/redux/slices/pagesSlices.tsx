    ...
    reducers: {
        addPage: (state) => {
            ...
            if (state.find((page)=> page.isLoaded === false)) {
                return state.map(({id, offset, isLoaded}) => isLoaded === false
        ...
        setPageLoaded: (state, action: PayloadAction<IPageLoaded>) => {
            return state.map(({id, offset, isLoaded}) => offset === action.payload.offset
            ? {id, offset, isLoaded: true, itemsCount: action.payload.itemsCount,}
            : {id, offset, isLoaded});
        },
        setPageNotLoaded: (state, action: PayloadAction<number>) => {
            return state.map(({id, offset, isLoaded, itemsCount}) =>
            offset === action.payload ? {id, offset, isLoaded: false}
...
export const {addPage, setPageLoaded, setPageNotLoaded, resetPages} = pagesSlice.actions;
